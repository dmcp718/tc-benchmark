...
"""
    ...
    def install_systemd_service(self) -> bool:
        ...
        service_path = self.script_dir / "lucid-site-cache.service"
        
        if not service_path.exists():
            console.print("[#f59e0b]Warning: lucid-site-cache.service not found[/#f59e0b]")
            return False
        
        try:
            # Check if service is already installed and running
            result = subprocess.run(
                ['systemctl', 'is-active', 'lucid-site-cache.service'],
                capture_output=True, text=True
            )
            ...
            if result.stdout.strip() == 'active':
                ...
                subprocess.run(['systemctl', 'stop', 'lucid-site-cache.service'], check=True)
                console.print("  ✓ Stopped existing service")
            
            # Check if service file exists and compare
            target_path = Path('/etc/systemd/system/lucid-site-cache.service')
            ...
            # Enable service
            subprocess.run(['systemctl', 'enable', 'lucid-site-cache.service'], check=True)
            console.print("  ✓ Enabled service to start on boot")
            
            # Start service
            subprocess.run(['systemctl', 'start', 'lucid-site-cache.service'], check=True)
            console.print("  ✓ Started lucid-site-cache.service")
            ...
            with Progress(
                ...
                for attempt in range(max_attempts):
                    ...
                    result = subprocess.run(
                        ['systemctl', 'status', 'lucid-site-cache.service', '--no-pager'],
                        capture_output=True, text=True
                    )
...
Install and start the systemd service:
[#3b82f6]sudo cp lucid-site-cache.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable lucid-site-cache.service
sudo systemctl start lucid-site-cache.service[/#3b82f6]
...
This tool will help you:
  ...
  • Install the systemd service"""
        ...
        if not license_path.exists():
            ...
            console.print("and place it in: [#93bbfc]/opt/sitecache/varnish-enterprise.lic[/#93bbfc]\n")
